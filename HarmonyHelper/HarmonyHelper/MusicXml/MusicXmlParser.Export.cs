...
namespace Eric.Morrison.Harmony.MusicXml
{
    public partial class MusicXmlExportr_Export
    {
        ...
        public bool Export(MusicXmlParsingResult model)
        {
            ...
            this.Document = XDocument.Load(xml);
        ...
        bool ExportImpl(MusicXmlParsingResult model)
        {
            ...
            //this.PreExportTiedNotes(this.Document);
...
#warning FIXME:            
            var parts = this.ExportParts(doc).Skip(1).ToList();
            foreach (var part in parts)
            {
                ...
                {
                    var measure = this.ExportMeasure(xmeasure);
        ...
        private void PreExportTiedNotes(XDocument doc)
        {
            ...
            var seq = (from xn in xnotes
                       ...
                       select new
                       {
                           ...
                           TieType = this.ExportTie(xn),
                           Note = Export_HarmonyHelper_Note(xn),
                           XNote = xn,
                       })
               .ToList();
            ...
            foreach (var xnote in xnotes)
            {
                var tieType = this.ExportTie(xnote);
        ...
        MusicXmlScoreMetadata ExportScoreMetadata(MusicXmlParsingResult model)
        {
#if false
   <measure number="1">
      <attributes>
      ...
      <forward>
         <duration>480</duration>
            ...
            result.Title = this.ExportTitle(doc);
            result.KeySignature = this.ExportKeySignature(doc);
            result.TimeSignatue = this.ExportTimeSignature(doc);
            result.Tempo = this.ExportTempo(doc);
            result.PulsesPerQuarterNote = this.ExportPpqn(doc);
        ...
        int ExportPpqn(MusicXmlParsingResult model)
        {//<divisions>120</divisions>
        ...
        int ExportTempo(MusicXmlParsingResult model)
        {//<sound tempo="160"/>
            var result = Int32.Parse(
        ...
        string ExportTitle(MusicXmlParsingResult model)
        {
            var result = doc.Descendants(XmlConstants.work_title).FirstOrDefault()?.Value;
            return result;
        }

        TimeSignature ExportTimeSignature(MusicXmlParsingResult model)
        {
#if false
        <time>
           <beats>4</beats>
        ...
        KeySignature ExportKeySignature(MusicXmlParsingResult model)
        {
        ...
        List<PartIdentifier> ExportPartList(MusicXmlParsingResult model)
        {
#if false
  <part-list>
   <score-part id="P1">
      <part-name>Bass</part-name>
...
#if true
        List<MusicXmlPart> ExportParts(MusicXmlParsingResult model)
        {
        ...
        private void PopulateMeasure(List<XElement> elements, MusicXmlMeasure measure)
        {
            ...
            foreach (var xelement in elements)
            {
                ...
                {
                    var duration = int.Export(xelement.Element(XmlConstants.duration).Value);
                ...
                {
                    var chord = this.ExportChord(xelement, chords);
                ...
                {
                    var note = this.ExportPitched(xelement);
                    if (note != null) ;
                    notes.Add(note);
                }
                ...
                {
                    var rest = this.ExportRest(xelement);
...
#if false
        MusicXmlPart ExportPart(XElement part)
        {
#if false
    <part id="P1">
      <attributes>
      ...
      <forward>
...
#endif
            var result = MusicXmlPart.Export(part);

            var measures = part.Descendants(XmlConstants.measure);
            foreach (var measure in measures)
            {
                result.Measures.Add(
                    this.ExportMeasure(measure));
            }
            return result;
        }
        ...
        List<XElement> GetMusicalElements(XElement part)
        {
            ...
            foreach (var measure in measures)
            {
                ...
                foreach (var element in elements)
                {
                    ...
                    {
                        var duration = this.ExportDuration(element);
                        this.ParsingContext.CurrentOffset -= duration;
                    }
                    else if (element.Name == XmlConstants.harmony)
                    {
                        result.Add(element);
                        //result.Add(this.ExportHarmony(element));
                    }
                    else if (element.Name == XmlConstants.note)
                    {
                        result.Add(element);
                        //result.Add(this.ExportNote(element));
                    }
                }
            }
        ...
        MusicXmlMeasure ExportMeasure(XElement xmeasure)
        {
            ...
            //    else if (xelement.Name == XmlConstants.harmony)
            //    {
            //        result.Add(this.ExportHarmony(xelement));
            //    }
            //    else if (xelement.Name == XmlConstants.note)
            //    {
            //        result.Add(this.ExportNote(xelement));
            //    }
            //}
            return result;
        }
        ...
        [Obsolete("", true)]
        TimedEvent<Note> ExportNote(XElement xnote)
        {// https://www.w3.org/2021/06/musicxml40/musicxml-reference/elements/note/
         //Debug.WriteLine($"{note}");
            ...
            if (note.Descendants("pitch").Any())
            {
                ...
                var nn = this.ExportPitch(xpitch);
        ...
        private TimedEvent<Note> ExportPitched(XElement xnote)
        {
            TimedEvent<Note> result = null;
            ...
            if (xnote.Elements(XmlConstants.pitch).Any())
            {
                hhNote = this.Export_HarmonyHelper_Note(xnote);
            }
            if (xnote.Elements(XmlConstants.type).Any())
            {
                duration = this.ExportDuration(xnote);
        ...
        TieTypeEnum ExportTie(XElement note)
        {
#if false
<note attack="18">
  ...
  <tie type="start" />
        ...
        private void ExportUnpitched(XElement xnote)
        {
        ...
        TimedEvent<Rest> ExportRest(XElement xnote)
        {
            TimedEvent<Rest> result = null;
            var duration = ExportDuration(xnote);
        ...
        public int ExportDuration(XElement xnote)
        {//The <duration> element moves the musical position when used in <backup> elements, <forward> elements, and <note> elements that do not contain a <chord> child element.
            ...
            if (xnote.Elements(XmlConstants.time_modification).Any())
            {
                var xtime_modification = xnote.Elements(XmlConstants.time_modification)
                    .First();
                var tm = this.ExportTimeModification(xtime_modification);
                result = tm.GetDuration(result);
            }
            return result;
        }

        TimeModification ExportTimeModification(XElement xtime_modification)
        {
        ...
        public Note Export_HarmonyHelper_Note(XElement note)
        {
#if false
  <pitch>
    <step>B</step>
    ...
    <octave>1</octave>
            ...
            if (NoteNameExporter.TryExport(strNoteName, out var notes, out var msg))
            {
                var nn = notes.First();
        ...
        TimedEvent<ChordFormula> ExportChord(XElement harmony, List<TimedEvent<ChordFormula>> existingChords)
        {
#if false
      <harmony>
         <root>
      ...
      <offset>240</offset>
            ...
            var strRoot = this.ExportRoot(root);

            var kind = harmony.Descendants(XmlConstants.kind).First();
            var formula = this.ExportKind(strRoot, kind);
            ...
            {
                start = ParsingContext.CurrentOffset + int.Export(
        ...
        ChordFormula ExportKind(string root, XElement kind)
        {
#if false
  ...
  <kind>major</kind>
        ...
        string ExportRoot(XElement root)
        {
#if false
  <root>
    ...
    public static class ModelToXElementExtensions
    {
        public static XElement ToPitch(this Note note)
        {
#if false
  <pitch>
    <step>B</step>
    ...
    <octave>1</octave>
            ...
            XElement step= new XElement(XmlConstants.step, note.Name[0]);
            result.Add(step);
        ...
        public static XElement ToHarmony(this TimedEvent<ChordFormula> te)
        {
#if false
      <harmony>
         <root>
      ...
      <offset>240</offset>
        ...
        public static XElement ToRoot(this TimedEvent<ChordFormula> te)
        {
#if false
  <root>
    <root-step>B</root-step>
    <root-alter>-1</root-alter>
        ...
        public static XElement ToXElement(this IHasTimeContext<ChordFormula> te)
        {
            throw new NotImplementedException();
#if false
      <harmony>
      ...
      <offset>240</offset>
        ...
        public static XElement ToXElement(this IHasTimeContext<Note> te)
        {
            throw new NotImplementedException();
#if false
      <harmony>
      ...
      <offset>240</offset>
        ...
        public static XElement ToXElement(this IHasTimeContext<Rest> te)
        {
            throw new NotImplementedException();
#if false
      <harmony>
